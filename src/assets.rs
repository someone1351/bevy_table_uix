...
fn input_def() -> conf_lang::Def {
    conf_lang::Def::new()
        .branch("root_branch").include(["apply_branch","nodes_branch"])
            .tags(["template"])
                .entry_children("node_branch").entry_label("template_decl")
                    .param_any()
            .tags(["include"])
                .entry()
                    .param_any()
            .tags(["script"])
                .entry_text().entry_label("is_root")
        ...
        .branch("node_branch").include(["nodes_branch","apply_branch","attribs_branch"])  //.include(["node2_branch"])
            .tags(["template"])
                .entry().entry_label("template_use")
                    .group_repeat()
                    .param_any()
            .tags(["script"])
                .entry_text().entry_label("not_root")
            // .tags(["mut"])
        ...
        .branch("attribs2_branch")
            .tags([
                ...
                .entry().entry_label("colori")
                    .param_parse::<u8>()
                    .param_parse::<u8>()
                    .param_parse::<u8>()
                    .param_optional()
                    .param_parse::<u8>()
                .entry().entry_label("colorf")
                    .param_parse::<f32>()
                    .param_parse::<f32>()
                    .param_parse::<f32>()
                    .param_optional()
