...
pub fn col_to_script_value(col:Color) -> Value {
    ...
    Value::custom_unmanaged_mut(Vec4::from_array(col.to_srgba().to_f32_array()))
}
pub fn script_value_to_col(val:Value) -> Result<Color,MachineError> {
    let v=val.as_custom();

    if let Some(v)=v.get_data_clone::<Vec4>()? {
        let v=v.map(|x|x.clamp(0.0,1.0));
        Ok(Color::srgba(v[0],v[1],v[2],v[3]))
    } else if let Some(v)=v.get_data_clone::<IVec4>()? {
        let v=v.map(|x|x.clamp(0,255));
        Ok(Color::srgba_u8(v[0] as u8,v[1] as u8,v[2] as u8,v[3] as u8))
     } else {
